...
public class Helpers {
    ...
    //constants
    public final static int DEFAULT_NUM_POSTS = 15;
    public final static int DEFAULT_NUM_USERS = 25;
    public final static int DEFAULT_NUM_COMMENTS = 10;
    public final static int DEFAULT_NUM_REACTIONS = 20;
    public final static int DEFAULT_NUM_TAGS = 15;
    ...
    public Post generatePost(User author, boolean savePost){
        ...
        Post post = new Post();
        post.setContent(faker.lorem().paragraph(ThreadLocalRandom.current().nextInt(5, 10)));
        if(author != null){
        ...
        if(savePost)
            post = postDataManager.save(post);
        return post;
    }
    ...
    public Set<Post> generatePosts(User author, int numPosts, boolean savePosts){
        ...
        if(author != null && author.getPosts() == null) author.setPosts(new HashSet<>());
        for(int i = 0; i < numPosts; i++) {
            if(author != null) posts.add(generatePost(author, false));
            else posts.add(generatePost(false));
        }
        if(savePosts)
            posts = new HashSet<>(postDataManager.saveAll(posts));
        return posts;
    }
    ...
    public Comment generateMockComment(User author, Post post, boolean saveComment){
        return generateMockComment(author, post, null, saveComment);
    }
    public Comment generateMockComment(User author, Post post, Comment inReplyTo, boolean saveComment){
        ...
        if(inReplyTo != null) {
            if(inReplyTo.getReplies() == null)
                inReplyTo.setReplies(new HashSet<>());
            inReplyTo.getReplies().add(comment);
        }
        comment.setContent(new Faker().lorem().paragraph(ThreadLocalRandom.current().nextInt(5, 10)));
        if(saveComment)
            comment = commentDataManager.save(comment);
        return comment;
    }
