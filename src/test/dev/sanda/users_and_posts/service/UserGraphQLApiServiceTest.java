...
class UserGraphQLApiServiceTest {
    ...
    void countTotalUsers() {
        ...
        for (int i = 0; i < toArchive; i++)
            users.get(i).setIsArchived(true);
    ...
    void getUsersById() {
        ...
        final String methodName = "getUsersById";

        // case 1 - all ids are valid
        List<User> users = helpers.generateMockUsers(numUsers, true);
        List<Long> validIds = users.stream().map(User::getId).collect(Collectors.toList());
        List<User> fetchedUsers = testApi.invokeEndpoint(methodName, validIds);
        Map<Long, User> fetchedUsersById = fetchedUsers.stream().collect(Collectors.toMap(User::getId, user -> user));
        for(User user : users)
            assertEquals(user, fetchedUsersById.get(user.getId()));

        // case 2 - all ids are invalid
        List<Long> badIds = new ArrayList<>();
        for(int i = 0; i < numUsers; i++) badIds.add(IdFactory.getNextId());
        List<User> shouldBeEmpty = testApi.invokeEndpoint(methodName, badIds);
        ...
        List<User> shouldContainNumUsers = testApi.invokeEndpoint(methodName, mixedIds);
        assertEquals(numUsers, shouldContainNumUsers.size());
    }
    ...
    void createUser() {
        ...
        User newlyCreatedUserLoadedFromDataBase = userDataManager.findById(newlyCreatedUser.getId()).orElse(null);
    ...
    void createUsers() {
        ...
        for (int i = 0; i < numUsers; i++) usersToCreate.add(helpers.generateMockUser());
        List<User> newlyCreatedUsers = testApi.invokeEndpoint(methodName, usersToCreate);
        ...
        for (int i = 0; i < numUsers; i++) mixedUserInputs.add(helpers.generateMockUser());
        assertThrows(
                InvalidDataAccessApiUsageException.class,
    ...
    void updateUsers() {
        ...
        for(User user : updatedUsers)
            assertEquals(usersById.get(user.getId()), user);
    }
    ...
    void deleteUsers() {
        ...
        assertFalse(deletedUsers.stream().anyMatch(user -> userDataManager.existsById(user.getId())));
    }
    ...
    void archiveUser() {
        ...
        User fetchedUser = userDataManager.findById(archivedUser.getId()).orElse(null);
    ...
    void archiveUsers() {
        ...
        List<User> fetchedArchivedUsers = userDataManager.findAllById(users.stream().map(User::getId).collect(Collectors.toList()));
    ...
    void deArchiveUser() {
        ...
        User fetchedDeArchivedUser = userDataManager.findById(user.getId()).orElse(null);
    ...
    void deArchiveUsers() {
        ...
        List<User> deArchivedUsers = testApi.invokeEndpoint("deArchiveUsers", users);
        assertFalse(deArchivedUsers.stream().anyMatch(User::getIsArchived));
        List<User> fetchedDeArchivedUsers = userDataManager.findAllById(users.stream().map(User::getId).collect(Collectors.toList()));
    ...
    void posts() {
        ...
        for(User user : users)
            helpers.generatePosts(user, 5, true);
        users = userDataManager.saveAll(users);
        List<List<Post>> postsLists = testApi.invokeEndpoint("posts", users);
        for (int i = 0; i < 5; i++){
            User user = users.get(i);
            Map<Long, Post> postsById = user.getPosts().stream().collect(Collectors.toMap(Post::getId, p -> p));
            for(Post post : postsLists.get(i))
                assertEquals(postsById.get(post.getId()), post);
        }
    }
    ...
    void associatePostsWithUser() {
        ...
        List<Post> associatedPosts = testApi.invokeEndpoint("associatePostsWithUser", user, postsList);
        assertEquals(postsList, associatedPosts);
        user = userDataManager.findById(user.getId()).orElse(null);
        assertNotNull(user);
        Map<Long, Post> postsOfUserById = user.getPosts().stream().collect(Collectors.toMap(Post::getId, p -> p));
        for(Post post : associatedPosts)
            assertEquals(postsOfUserById.get(post.getId()), post);
    }
    ...
    void removePostsFromUser() {
        ...
        testApi.invokeEndpoint("removePostsFromUser", user, new ArrayList<>(posts).subList(0, 5));
        user = userDataManager.findById(user.getId()).get();
    ...
    void postsOfUserFreeTextSearch() {
        ...
        String searchTerm = helpers.getLongestCommonSubstring(posts.stream().map(Post::getContent).collect(Collectors.toSet()));
        FreeTextSearchPageRequest request = new FreeTextSearchPageRequest();
        request.setPageNumber(0);
        request.setPageSize(20);
        request.setSearchTerm(searchTerm);
        Page<Post> response = testApi.invokeEndpoint("postsOfUserFreeTextSearch", user, request);
        assertEquals(0, response.getPageNumber());
