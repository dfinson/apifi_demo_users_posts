...
class PostGraphQLApiServiceTest {
    ...
    void comments() {
        User user = helpers.generateMockUser(true);
        List<Post> posts = new ArrayList<>(helpers.generatePosts(user, DEFAULT_NUM_POSTS, true));
        List<List<Comment>> commentsOfPosts = new ArrayList<>();
        for (Post post : posts) {
            List<Comment> comments = new ArrayList<>();
            for (int i = 0; i < DEFAULT_NUM_COMMENTS; i++)
                comments.add(helpers.generateMockComment(user, post, true));
            commentsOfPosts.add(comments);
        }

        List<List<Comment>> fetchedCommentsOfPosts = testApi.invokeEndpoint("comments", posts);
    ...
    void addReactionsToPost() {
        ...
        assertThrows(RuntimeException.class,
                () -> testApi.invokeEndpoint("addReactionsToPost", post, reactions));
        post.setReactions(new ArrayList<>());
        List<Reaction> addedReactions = testApi.invokeEndpoint("addReactionsToPost", post, reactions);
    ...
    void removeReactionsFromPost() {
        ...
        List<Reaction> removedReactions = testApi.invokeEndpoint("removeReactionsFromPost", post, reactions);
    ...
    void reactionsOfPost() {
        ...
        Page<Reaction> response = testApi.invokeEndpoint("reactionsOfPost", post, request);
    ...
    void freeTextSearchReactionsOfPost() {
        ...
        request.setSearchTerm(
            helpers.getLongestCommonSubstring(reactions.stream().map(Reaction::toString).collect(Collectors.toSet()))
        );
        request.setPageSize(DEFAULT_NUM_REACTIONS);
        request.setPageNumber(0);
        Page<Reaction> response = testApi.invokeEndpoint("freeTextSearchReactionsOfPost", post, request);
        assertEquals(0, response.getPageNumber());
        assertEquals(numReactions, response.getTotalItemsCount());
        assertEquals(3, response.getTotalPagesCount());
        assertEquals(DEFAULT_NUM_REACTIONS, response.getContent().size());
    ...
    void addTagCountToPost() {
        ...
        Arrays.stream(Tag.values()).forEach(tag -> tagMap.put(tag, DEFAULT_NUM_TAGS));
        Map<Tag, Integer> addedTagCounts = testApi.invokeEndpoint("addTagCountToPost", post, tagMap);
    ...
    void removeTagCountFromPost() {
        ...
        Map<Tag, Integer> removed = testApi.invokeEndpoint("removeTagCountFromPost", post, toRemove);
        Map<Tag, Integer> expected = new HashMap<Tag, Integer>(){{put(Tag.INSPIRATIONAL_, DEFAULT_NUM_TAGS);}};
    ...
    void tagCountOfPost() {
        ...
        Page<Map.Entry<Tag, Integer>> response = testApi.invokeEndpoint("tagCountOfPost", post, request);
    ...
    void freeTextSearchTagCountOfPost() {
        ...
        Page<Map.Entry<Tag, Integer>> response = testApi.invokeEndpoint("freeTextSearchTagCountOfPost", post, request);
